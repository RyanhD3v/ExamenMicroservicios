...
public class RabbitConfigConsumer {
	...
	public final static String QUEUE_USER_CONFIRMACION = "queue-user-confirmacion";
	...
	@Bean
	Queue queue2() {
		return new Queue(QUEUE_USER_CONFIRMACION);
	}
	...
	Binding binding2(Queue queue, TopicExchange exchange) {
		return BindingBuilder.bind(queue).to(exchange).with(QUEUE_USER_CONFIRMACION);
	}
}
